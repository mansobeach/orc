...
# == Synopsis
#
# This is a SMOS NRTP Orchestrator command line tool that deletes a running job from database. 
# 
# 
# -P flag:
#
...
#
# == Usage
# deleteRunningJob.rb -P <processor-id> -I <joborder-id> 
#
...
#
# CVS: $Id: deleteRunningJob.rb,v 1.1 2008/07/29 10:48:08 decdev Exp $
#
#########################################################################
...
def main
   ...
   arrJobs = RunningJob.find(:all, :conditions => ["proc_id=(?) and joborder_id=(?)", @processor_id, @joborder_id])
   
   if arrJobs.size() > 0 then
      RunningJob.destroy(arrJobs)
   else
      puts
      puts "Unregistered joborder !"
      puts
      exit(99)
   end
